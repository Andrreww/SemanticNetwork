...
#  # 开启词性标注功能
#text = seg.cut('我送给她一束花')    # 进行分词和词性标注
#print(text)
...
def build_semantic_net_zh(text, display=False, display_name='SemanticNet'):
    """
    :param text: input sentences or article
    :param display: to display the semantic network in browser, or not
    :param display_name: the folder name of output html file;

    :return: a semantic network built according to the input
    """
    try:
        line = dict()
        desc = dict()
        ...
        for sent in sentences:
            ...
            s, v, o ,last= False, False, False,False
...
#                if word not in stopwords:
                ...
                if pos[0] == 'r':
                    if last is not False:
                        s = last
                if pos == 'v' and word not in stopwords:
                    if s is not False or last is not False:
                        if s == False:
                            s = last
                        v = word
                ...
                if 'n' in pos and v != False:
                    if (s is not False) and (v is not False) and (o is False):
                        o = word
                    elif o == words[i-1][0] or o == words[i-2][0]:
                        o += word
                if i != len(words)-1 and words[i+1][0] == '的':
                    continue
...
line2 = build_semantic_net_zh(string_data, True)
rungraphorg(line)
rungraph(line)
rungraph2(line)
rungraph3(line)
